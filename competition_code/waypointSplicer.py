...
replacementWaypoints = roar_py_interface.RoarPyWaypoint.load_waypoint_list(
    np.load("competition_code\\waypoints\\waypoints11.npz")
)
...
def on_click(event):
    if event.button is MouseButton.LEFT:
        if len(baseSection) < 2:
            baseSection.append(findCurrentIndex([event.xdata, event.ydata], replacementWaypoints))
            print(f"Plotting source waypoint {len(baseSection)} at ({event.xdata}, {event.ydata})")
            plt.plot(event.xdata, event.ydata, "bs")
        # elif len(sourceSection) < 2 and not findTarget:
        #     sourceSection.append(findCurrentIndex([event.xdata, event.ydata], sourceWaypoints))
        #     print(f"Plotting target waypoint {len(sourceSection)} at ({event.xdata}, {event.ydata})")
        #     plt.plot(event.xdata, event.ydata, "bs")
        else:
            plt.close()
            for i in range(len(baseSection)):
                ...
                lowestDistance = distanceToWaypoint(currentLoc.location, replacementWaypoints[0])
                ...
                replacementSection.append(findCurrentIndex(closestWaypoint.location, replacementWaypoints))
                print(f"Target waypoint {i + 1} found at {closestWaypoint.location}, {lowestDistance} units away")
                    
            print(f"Source Waypoints: {len(baseSection)}\nTarget Waypoints: {len(replacementSection)}")
            
drawWaypoints(baseWaypoints)
# findTarget = True
# drawWaypoints(targetWaypoints, sourceSection)
...
np.savez_compressed("modifiedWaypoints.npz", **roar_py_interface.RoarPyWaypoint.save_waypoint_list(newWaypoints))